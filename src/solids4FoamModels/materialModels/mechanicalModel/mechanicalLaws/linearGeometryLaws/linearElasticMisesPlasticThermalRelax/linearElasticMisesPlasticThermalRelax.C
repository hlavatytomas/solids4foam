...
#include "linearElasticMisesPlasticThermalRelax.H"
...
namespace Foam
{
    defineTypeNameAndDebug(linearElasticMisesPlasticThermalRelax, 1);
    addToRunTimeSelectionTable
    (
        mechanicalLaw, linearElasticMisesPlasticThermalRelax, linGeomMechLaw
    );

// * * * * * * * * * * * * * * Static Members  * * * * * * * * * * * * * * * //

    // Tolerance for Newton loop
    scalar linearElasticMisesPlasticThermalRelax::LoopTol_ = 1e-8;

    // Maximum number of iterations for Newton loop
    label linearElasticMisesPlasticThermalRelax::MaxNewtonIter_ = 200;

    // finiteDiff is the delta for finite difference differentiation
    scalar linearElasticMisesPlasticThermalRelax::finiteDiff_ = 0.25e-6;

    // Store sqrt(2/3) as we use it often
    scalar linearElasticMisesPlasticThermalRelax::sqrtTwoOverThree_ = ::sqrt(2.0/3.0);
...
void Foam::linearElasticMisesPlasticThermalRelax::updatePlasticity
(
...
Foam::scalar Foam::linearElasticMisesPlasticThermalRelax::curYieldStress
(
...
Foam::scalar Foam::linearElasticMisesPlasticThermalRelax::yieldFunction
(
    const scalar epsilonPEqOld,    // Old equivalent plastic strain
...
void Foam::linearElasticMisesPlasticThermalRelax::newtonLoop
(
...
) const
{
    ...
    do
    {
        ...
        if (i == MaxNewtonIter_)
        {
            WarningIn("linearElasticMisesPlasticThermalRelax::newtonLoop()")
...
Foam::linearElasticMisesPlasticThermalRelax::linearElasticMisesPlasticThermalRelax
(
    const word& name,
    const fvMesh& mesh,
    ...
    n_("zero", dimless, 0.0)
{
    if (planeStress())
    {
        FatalErrorIn
        (
            "linearElasticMisesPlasticThermalRelax::linearElasticMisesPlasticThermalRelax::()"
    ...
    C_ = dimensionedScalar(dict.lookup("C"));
...
Foam::linearElasticMisesPlasticThermalRelax::impK() const
{
    ...
    const volSymmTensorField sTrial(2.0*mu_*(e - dev(epsilonP_.oldTime()) - dev(epsilonTR_.oldTime())));
    // const volSymmTensorField sTrial(2.0*mu_*(e - dev(epsilonP_.oldTime()) + (dev(epsilonTR_) - dev(epsilonTR_.oldTime()))));
    // const volSymmTensorField sTrial(2.0*mu_*(e - dev(epsilonP_.oldTime()) + (dev(epsilonTR_))));
...
Foam::linearElasticMisesPlasticThermalRelax::impKdiagTensor() const
{
    // Calculate scaling factor to ensure optimal convergence
    // This is similar to the tangent matrix in FE procedures
...
Foam::linearElasticMisesPlasticThermalRelax::materialTangentField() const
{
    // // Prepare tmp field
    ...
    // // Update total strain
    // const_cast<linearElasticMisesPlasticThermalRelax&>(*this).updateEpsilonf();
    ...
    // const surfaceSymmTensorField sTrial(2.0*mu_*(e - dev(epsilonPf_.oldTime()) + (dev(epsilonTR_) - dev(epsilonTR_.oldTime()))));
    ...
    //         const_cast<linearElasticMisesPlasticThermalRelax&>
    //         (
    //             *this
...
void Foam::linearElasticMisesPlasticThermalRelax::correct(volSymmTensorField& sigma)
{
    ...
    // DEpsilonTR_.replace(symmTensor::XZ, C_ / dimensionedScalar("dummy", dimPressure, 1) * dTimeSc * sigma.component(symmTensor::XZ) * sigma.component(symmTensor::XZ));
    ...
    const volSymmTensorField s(sTrial - 2*mu_*DEpsilonP_ - 2*mu_*DEpsilonTR_);
    ...
    // Update thermal relaxation strain
    // epsilonTR_ = epsilonTR_ + C_ * Foam::pow(sigma, n_);
}


void Foam::linearElasticMisesPlasticThermalRelax::correct(surfaceSymmTensorField& sigma)
{
    // Calculate total strain
    updateEpsilonf();
...
void Foam::linearElasticMisesPlasticThermalRelax::correct
(
...
Foam::tmp<Foam::volScalarField> Foam::linearElasticMisesPlasticThermalRelax::bulkModulus() const
{
...
Foam::scalar Foam::linearElasticMisesPlasticThermalRelax::residual()
{
...
void Foam::linearElasticMisesPlasticThermalRelax::updateTotalFields()
{
...
Foam::scalar Foam::linearElasticMisesPlasticThermalRelax::newDeltaT()
{
